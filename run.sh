...
create_dump() {
  docker-compose exec postgresql pg_dump  -U ${DB_USER} -w   -d ${DB_NAME} > $DUMP_DIR/dump.sql
}

restore_dump() {
  docker exec -i $(docker-compose ps -q postgresql) psql  -U ${DB_USER} -w -d ${DB_NAME} < $DUMP_DIR/dump.sql
}
